...
**Path** `src/windows/UI/TradingTerminal.UI.Core/` · **Editions** B I P · **Blast: high**

**Purpose.** The MVVM backbone every live window builds on: `ViewModelBase`,
**`LiveSignalStrategyViewModelBase`** (hub subscription pumps, pause/presets/CSV export,
...
strategy VMs), **`InMemoryLogSink`** (`Logging/InMemoryLogSink.cs` — the universal Activity Log),
`StrategyWindowBase`, `StrategyChromeBar` (binds by convention).

**Depends on** Core only. **Depended by** UI, Sdk.Wpf (→ installed UI strategy plugins), all chart/tool
projects, Settings, Tests.Headless.

**Surface** `symbols/UI.Core.md` — use it to inspect the base contract before opening the
implementation.
...
(design once here for installed plugins); chrome-bar additions; log-sink filtering.
