...
## src/windows/Pipeline/TradingTerminal.Infrastructure/Backtest/Worker/AbandonedWorkerStagingCleaner.cs
```cs
   12: public static int Cleanup(string jobRoot, TimeSpan minimumAge, DateTime utcNow)
```

## src/windows/Pipeline/TradingTerminal.Infrastructure/Backtest/Worker/BacktestJobClient.cs
```cs
   21: public sealed class BacktestJobClient : IBacktestJobClient, IDisposable
   29: public BacktestJobClient(
   36: public BacktestJobClient(
   44: public async Task<BacktestJobOutcome> RunAsync(
  425: public void Dispose()
  940: public Process Process { get; } = process;
  942: public bool TryAssign() => guard.TryAssign(Process);
  944: public void Dispose()
```

## src/windows/Pipeline/TradingTerminal.Infrastructure/Backtest/Worker/BacktestWorkerExecutableResolver.cs
```cs
   15: public static bool TryResolve(
```

## src/windows/Pipeline/TradingTerminal.Infrastructure/Backtest/Worker/BacktestWorkerOptions.cs
```cs
    3: public sealed class BacktestWorkerOptions
    9: public string? WorkerExecutablePath { get; set; }
   12: public List<string> WorkerArguments { get; } = [];
   15: public string? JobRootDirectory { get; set; }
   18: public TimeSpan DefaultTimeout { get; set; } = TimeSpan.FromMinutes(10);
   20: public int ProgressBufferCapacity { get; set; } = 32;
   21: public int MaxProgressLineCharacters { get; set; } = 16 * 1024;
   22: public int MaxCapturedStandardErrorCharacters { get; set; } = 64 * 1024;
   25: public TimeSpan AbandonedStagingAge { get; set; } = TimeSpan.FromDays(2);
```

## src/windows/Pipeline/TradingTerminal.Infrastructure/Backtest/Worker/BacktestWorkerServiceCollectionExtensions.cs
```cs
    5: public static class BacktestWorkerServiceCollectionExtensions
    7: public static IServiceCollection AddBacktestWorker(
```

## src/windows/Pipeline/TradingTerminal.Infrastructure/Backtest/Worker/IBacktestJobClient.cs
```cs
