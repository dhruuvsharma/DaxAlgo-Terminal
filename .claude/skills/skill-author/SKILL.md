...
- **`description`** — the *only* trigger signal. Put **what it does AND when to use it** here, not
  in the body. Claude tends to *under*-trigger skills, so make the description a little **pushy**:
...
- Keep it ASCII-safe if any bundled script is PowerShell (Windows 5.1 mangles non-ASCII in no-BOM
  files — see `.claude/hooks/verify-on-stop.ps1`).
...
- **Bespoke** (default): author it here, tailored to the repo. Everything in `.claude/skills/`
  today is bespoke.
...
#   in Claude Code:  /plugin   -> add marketplace -> install
...
Copy-Item $env:TEMP\ext-skill\skills\<name> .claude\skills\<name> -Recurse
...
- Don't ship a skill without wiring it to an agent or the CLAUDE.md table.
