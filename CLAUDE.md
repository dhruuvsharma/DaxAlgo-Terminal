...
Strategies.*   → build against the DaxAlgo SDK ONLY; runtime plugins via AddStrategyPlugins → PluginLoader (ADR-0008). MOVED OUT → DaxAlgo-Terminal-Strategies repo (not in this tree)
...
- **New strategy → the [DaxAlgo-Terminal-Strategies](https://github.com/dhruuvsharma/DaxAlgo-Terminal-Strategies) repo** (or author one in-app via the Roslyn / AI builder), shipped as a runtime `.daxplugin`: an `IBacktestStrategy` kernel wrapped by `LiveSignalStrategyViewModelBase` (in `UI.Core`), built against the SDK — **not** a project in this tree. `dotnet new daxalgo-strategy` scaffolds one.
...
**Strategy-vs-tool rule:** anything that registers an `ITradingStrategy` / `StrategyFactoryRegistration` (including multi-ticker *monitor* strategies) is a **strategy** — it ships as a runtime plugin (the [DaxAlgo-Terminal-Strategies](https://github.com/dhruuvsharma/DaxAlgo-Terminal-Strategies) repo, or an in-app authored `.daxplugin`), **not** a project in this tree. Tool projects (`Add…Surface`, Tools/Charts menu) are the only window projects that live here, and only for non-strategy windows. When in doubt: if it belongs in the strategy catalog, it's a plugin, not a tool project.

Per-tool projects: the shells don't host tool windows — each tool is its own `TradingTerminal.<Name>` project (under `src/windows/<Group>/`, grouped by **Charts** / **Tools** / **AI** / **Strategies** solution folders). Shells reference them and open them via `IServiceProvider`; each project ships its own `Add…Surface` extension called from the shell composition (`Composition/AppDependencyInjection.cs`). The Charts menu in Basic/Intermediate hosts Charts/OrderBook/VolumeFootprint/Heatmap (Bookmap); BubbleChart, 3D Surface Lab, and the Machine Learning menu (`Ml.*`: Stationarity & Differencing / ARIMA & GARCH / Kalman Filter) exist only in the Professional shell (private Pro repo) — their math stays here in `Core/Quant/TimeSeries/` and `Core/Quant/Surfaces/`.

**⚠️ Edition shells — NO shared shell code (2026-07-08, #19).** Three fully independent WinExe shells, now split across two repos: this repo ships `TradingTerminal.App.Basic` (**keyless brokers only** — crypto + Simulated, keyless login, core charts/tools, plugin-loaded strategy catalog — empty on a fresh install) and `TradingTerminal.App.Intermediate` (**all brokers + full login**, same tools as Basic) under `src/windows/Shell/`; the **Professional** shell `TradingTerminal.App` (everything) lives in the private Pro repo. There is no `App.Core` shared shell library — each shell owns a complete copy of the shell code (`RootNamespace` stays `TradingTerminal.App` in all three; they never reference each other), **so a shell-code fix must be applied to all three copies (two here + one in the Pro repo)**. Tier gating is composition-only: Basic never references the Pro-only tool projects, calls `AddKeylessBrokers()` without `AddCredentialedBrokers()`, and `AddLogin()` registers only keyless login forms — `AddCredentialedLoginForms()` must always pair with `AddCredentialedBrokers()` (resolving `IEnumerable<IBrokerLoginForm>` instantiates every registered form, so an unpaired credentialed form crashes the login window). `AppEdition`/`BrokerEditionPolicy` live in `Core/Configuration`; each shell pins its `AppEdition` as a DI constant. The Simulated broker ships in every edition behind the amber “SIMULATED DATA” banner (`SimulatedDataState`/`SimulatedDataBanner` in `TradingTerminal.UI`).
...
- Don't build a strategy as a tool project: nothing with an `ITradingStrategy` belongs in a `TradingTerminal.<Name>` + `Add…Surface` tool project — strategies ship as runtime plugins (the DaxAlgo-Terminal-Strategies repo, or an in-app authored `.daxplugin`; see the strategy-vs-tool rule above).
