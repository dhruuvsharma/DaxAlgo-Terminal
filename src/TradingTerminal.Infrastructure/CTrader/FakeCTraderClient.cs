...
/// <summary>
/// Synthetic cTrader client. Used when the user opts out of the real Open API client
/// (e.g. before they've set up Spotware OAuth credentials). Generates a plausible
/// random-walk price stream so the rest of the app behaves identically regardless of
/// the chosen broker.
/// </summary>
public sealed class FakeCTraderClient : IBrokerClient
{
    private readonly ILogger<FakeCTraderClient> _logger;
    ...
    private readonly Random _rng = new(13);
    private readonly Dictionary<string, double> _lastPrice = new(StringComparer.OrdinalIgnoreCase);
    private readonly object _gate = new();

    public FakeCTraderClient(ILogger<FakeCTraderClient> logger, IOptions<CTraderOptions> options)
    {
        _logger = logger;
        _options = options;
    }
    ...
    public async Task ConnectAsync(CancellationToken ct = default)
    {
        ...
        _logger.LogInformation("FakeCTraderClient connecting (host={Host}, account={Account})",
            opt.Host, opt.CtidTraderAccountId);
