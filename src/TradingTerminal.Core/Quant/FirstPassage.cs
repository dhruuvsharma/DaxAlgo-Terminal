...
{
    /// <summary>
    /// Win probability P(hit +b before −a). <paramref name="stop"/> = a &gt; 0 (distance below
    /// entry), <paramref name="target"/> = b &gt; 0 (distance above entry), drift <paramref name="mu"/>,
    /// vol <paramref name="sigma"/> (&gt; 0). Handles μ→0 analytically and guards overflow by
    /// working in the exponent space. Returns a value in [0, 1].
    /// </summary>
    public static double WinProbability(double stop, double target, double mu, double sigma)
    {
        var a = stop;
        var b = target;
