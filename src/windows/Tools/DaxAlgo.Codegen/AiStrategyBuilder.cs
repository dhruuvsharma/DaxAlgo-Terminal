...
public interface IAiStrategyBuilder
{
    ...
    /// the same context. The session never registers anything.</summary>
    StrategyBuildSession StartSession(IStrategyCodegenClient provider, string strategyId, string displayName);
    ...
    AiCodegenOptions options) : IAiStrategyBuilder
{
    ...
    public StrategyBuildSession StartSession(IStrategyCodegenClient provider, string strategyId, string displayName) =>
        orchestrator.CreateSession(provider, pack.SystemPrompt, strategyId, displayName, options.MaxFixAttempts);
