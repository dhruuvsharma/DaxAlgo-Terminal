...
public static class SdkSurfaceGenerator
{
    ...
    public static string Generate()
    {
        ...
        // The quant library lives in its own assembly now and DaxAlgo.Sdk only forwards to it. A
        // forwarded type is not an EXPORTED type of the forwarding assembly, so reading DaxAlgo.Sdk
        // alone would silently drop every estimator from the surface.
        var quant = typeof(DaxAlgo.Sdk.Quant.Num).Assembly;
        var docs = XmlDocumentation.LoadFor(assembly, quant, typeof(TradingTerminal.Core.Domain.InstrumentId).Assembly);
    ...
    internal static string Inline(string text) => text.Replace(XmlDocumentation.ParagraphMark, ' ');

    internal static string? Lead(string? summary)
    {
    ...
    internal static bool IsInteresting(MemberInfo member) => member switch
    {
    ...
    internal static string Signature(MemberInfo member) => member switch
    {
    ...
    internal static string MemberKey(MemberInfo member) => member switch
    {
        Type t => $"T:{t.FullName}",
