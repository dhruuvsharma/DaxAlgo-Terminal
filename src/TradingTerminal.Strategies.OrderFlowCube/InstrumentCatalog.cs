...
/// <summary>A user-facing instrument label paired with the broker contract and the source broker.
/// <see cref="Broker"/> is null for static-catalog rows; the strategy host falls back to the first
/// connected broker at Start time.</summary>
...
/// <summary>
/// Static fallback catalog used until the connected broker's list resolves. Focused on instruments
/// that have a meaningful centralized trade tape under typical IB market-data subscriptions
/// (US equities, ETFs, equity-index futures). FX/crypto omitted — they don't broadcast a
/// centralized tape so CVD/aggressor signals are noise-dominated.
/// </summary>
